          ...
          <div class="card-header" style="background: #5A9599;color: #fff">
            <h3>New Patient Appointment</h3>
                           ...
                           <div style="margin-top: 25px">
                                ...
                                <a href="{{route('admin.appointment.new')}}" class="btn btn-info margin-5"  style="float: right;">
                                    Add New Appointment
                                </a>
                           </div>
                            <!-- Tab panes -->
                                    ...
                                    <table id="myTable" class="table table-striped table-bordered">
                                        <thead>
                                          ...
                                          <tr>
                                            ...
                                            <td>
                                                <a href="{{route('admin.prescribe.new',[$all->id,$all->pa_id])}}" class="btn btn-danger">Prescribe</a>
                                            </td>
...
<div class="row"> 
   <div class="col-md-12">
         <div class="card">
          <div class="card-header" style="background: #5A9599;color: #fff">
            <h3>Old Patient Appointment</h3>

          </div>
                           <div style="margin-top: 25px">
                                 <button type="button" class="btn btn-success margin-5" data-toggle="modal" data-target="#Modal1">
                                 Old Appointment List
                                </button>
                         
                                </button>
                                <!-- Button trigger modal -->
                                <a href="{{route('admin.appointment.old')}}" class="btn btn-info margin-5"  style="float: right;">
                                    Add Old Appointment
                                </a>
                           </div>
                            <!-- Tab panes -->
                                  ...
                                  <div class="table-responsive">
                                    <table id="myTable1" class="table table-striped table-bordered">
                                        <thead>
                                          ...
                                          @foreach($old as $oldall)
                                          <tr>
                                            <td>{{$oldall->pat_f_name}}</td>
                                            <td>{{$oldall->pa_id}}</td>
                                            <td>{{$oldall->ap_mobile}}</td>
                                            <td>{{$oldall->ap_date}}</td>
                                            <td><?php echo date('D',strtotime($oldall->ap_date)).' day' ?></td>

                                            <td>{{$oldall->serial_no}}</td>
                                            <td>{{$oldall->appointed_by}}</td>
                                            <td>
                                              @if($oldall->payment_status=='paid')
                                              <span class="badge badge-pill badge-success">Paid</span>
                                              @else
                                              <span class="badge badge-pill badge-danger">UnPaid</span>
                                              @endif
                                            ...
                                            <td >
                                               <a href="" class="btn btn-primary margin-5 edit-oap" data-toggle="modal" data-target="#editmodal"  oap_id="{{$oldall->id}}">Print</a>
                                              
                                            </td>
                                            <td> <a href="{{route('admin.prescribe.old',[$oldall->id,$oldall->pa_id])}}" class="btn btn-danger">Prescribe</a></td>
  ...
  $(".edit-ap").click(function(){
    var ap_id =$(this).attr('ap_id');
          ...
          $.ajax({
              ...
              type: 'POST',
              url: "{{URL::to('/admin/newapoint/neapprint')}}",
              data : {ap_id:ap_id},
              dateType: 'json',
